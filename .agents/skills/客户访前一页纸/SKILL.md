...
>
> 核心能力：
> - 综合通过准入核验的内部事实与网络证据，按“企业简介、重点发展方向、风险提示、拜访目标”形成自然、可核验的核心观点；企业简介不以内部主体描述为前置底座，企业沿革仅在证据充分时单列第二自然段，核心观点正文不显示来源归因话术
...
**SKILL 版本**：v5.2-source-free-core-profile ｜ **适配数据源**：连接标识为 `cisp-mcp` 的水滴征信 MCP 当前工具版本 + AI 网络搜索的全部来源与候选 ｜ **纳入模式**：全量纳入、分层标注、不设数量上限 ｜ **外部窗口**：近期检索默认 12 个月，同时不限期检索历史、人物、股东、资产与风险 ｜ **页面规格**：Letter 215.9 × 279.4 mm ｜ **默认交付**：PDF，失败回退 Markdown
