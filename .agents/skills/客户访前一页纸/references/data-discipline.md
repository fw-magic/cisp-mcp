...
16. 先按固定规则构建 `D.company_overview_facts` 内部事实白名单和 `D.company_overview_fallback` 确定性回退文本，再由大模型仅在白名单内生成 `D.core_internal_baseline`；AI 只能改变取舍、语序和连接方式，不得修改原值、吸收外部信息或增加推断。`D.core_internal_baseline` 仅保留为内部主体描述和回退资产，不是 `D.core_company_profile` 的必选输入、起笔底座或生成前置。生成企业简介前必须另行构建 `D.core_profile_candidate_facts`；成立时间、登记状态、注册资本、经营范围、注册地址、登记行业、法定代表人等工商字段只作为低优先级可选候选，不要求进入简介。除为明确主语使用规范企业名称外，不得把工商字段作为企业简介的固定底座。外部资料不得静默覆盖内部标准字段；空字段、非数字风险字段、空结果和失败维度不得改写成“无”。
