...
2. 先完成全部开放台账再生成核心观点。核心观点引用所有高相关 OPP/RISK；引用 OPP 时必须根据 `opportunity_type` 区分贷款机会、融资线索与综合金融机会，引用 RISK 时必须根据 `risk_type` 区分已核实风险、风险观察与待核实事项。正文过长时概括分组并列出完整编号范围，后文保留全量明细。
...
5. `D.core_risk_constraint` 可以使用三种风险类型的 RISK、资料缺口和模型分析，说明其可能影响产品准入、担保结构、资料要求或推进顺序；允许给出分析等级，但不得冒充银行审批结论。
6. `D.core_visit_focus` 必须落到需求金额、贷款用途、期限、还款来源、现有融资、增信资源、风险事项当前状态和决策流程中的 2 至 3 项。
7. 全文 260 至 420 字、5 至 8 句。`EVIDENCE.core_*` 与 `source_attributions.core` 必须逐句闭环。核心观点可以提出“首选从某产品族场景切入”，但必须同时出现“待核验”边界，禁止承诺授信结果。
...
先完成内部原值、外部候选、来源正文、风险归一化和事实表，再按“内部主体描述 → 统一 OPP 台账及机会类型 → 统一 RISK 台账及风险类型 → 核心观点 → 执行摘要 → 逐项营销方案 → 风险深化 → 按编号生成拜访脚本”的顺序生成。发现新证据时允许重建台账并同步全部编号引用。
