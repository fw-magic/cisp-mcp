...
1. 生成 `D.core_company_profile` 前，必须先构建 `D.core_profile_candidate_facts`。候选池汇总内部事实、`WEB.sources` 正文和 `WEB.candidates` 搜索摘要，按 `positioning → honor → brand_product → business_foundation → operating_scale → current_stage → registry_support` 分类；每项保存 `CPF-xx`、候选原文、主体、时点、证据状态、置信度、关系路径、来源引用、限制、选择优先级和是否采用。只删除完全重复、与目标无关、含受保护敏感信息或明显恶意内容；主体歧义和原文不足的候选仍保留，但标记 `ambiguous/unverified` 且不视为合格候选。
2. 合格候选指 `fact_type` 属于 `positioning/honor/brand_product/business_foundation/operating_scale/current_stage`，且 `relationship_path=direct|verified_alias`。荣誉、称号、品牌和行业身份只要正文或搜索摘要确有对应表述即可合格，不要求打开正文、直接来源、高可靠来源或交叉验证；包含“唯一”“第一”“名酒”“龙头”等宣传性或评价性词语的称号也可直接入池。弱候选保留原证据状态和限制，正文可用“被称为”“被列为”等审慎口径，但不得凭模型记忆补造或把称号推导为经营质量、财务实力、市场份额、偿债能力或授信结论。
...
4. `D.core_company_profile` 不以 `D.core_internal_baseline` 为必选输入、起笔底座或生成前置，固定按“定位结论＋代表性荣誉/品牌/产品＋业务基础、经营规模或当前阶段”组织为一个自然段。开头直接回答“这家公司是什么、靠什么被识别”，随后选 1 至 2 项最有区分度的荣誉、品牌或产品，再用 1 至 2 项业务基础、规模数据或当前经营状态收束。不得逐项罗列候选池。
5. 工商数据不是企业简介的必要显示内容。除为明确主语可自然使用规范企业名称外，登记状态、成立时间、曾用名、注册资本、实缴资本、经营范围、注册地址、登记行业、法定代表人等 `registry_support` 字段均为低优先级可选补充；只有能增强主体识别或候选池缺少更有区分度的事实时才采用，不得固定起笔、固定凑项或取代荣誉、品牌、产品和业务基础。
6. `D.core_history_evolution` 按时间先后组织成立、并购、重整、上市退市、重大股权变化、重大项目转折和当前状态，通常写成一个自然段。只有存在至少两个可以可靠排序的关键节点，且能说明当前状态时才生成；否则保持 `null`，骨架整段省略，不以资料不足句占位。
...
14. `EVIDENCE.core_profile_candidate_facts` 逐项登记完整候选池，`EVIDENCE.core_company_profile` 必须逐句回指实际采用的 `CPF-xx` 及底层来源；`core_history_evolution`、`core_development_direction`、`core_risk_prompt`、`core_visit_objective` 分别逐句映射实际证据，`source_attributions.core` 汇总实际使用的内部维度与 `Wn/Cn`。任一字段验收失败时只重写该字段，不重新搜索、不补造事实；历史沿革验收失败则置为 `null`。
