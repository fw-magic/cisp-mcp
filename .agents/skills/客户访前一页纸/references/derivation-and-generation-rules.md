...
1. `D.core_company_profile` 以已验收的 `D.core_internal_baseline` 为底座，从企业定位、代表性荣誉、品牌、产品、许可、资质或业务基础中选择最有区分度的事实形成一个自然段。企业名称、荣誉和品牌必须有直接证据；宣传性称号只能带来源归因，不得改写为独立事实。
...
6. 五个展示字段不得出现 OPP/RISK 编号、Markdown 标题或定位说明。`D.core_opportunity_ids` 和 `D.core_risk_ids` 仅作为生成与证据验收的内部回指，完整编号索引继续由执行摘要承担。
7. 语言以企业名称、明确时间、具体动作和结果组织，避免连续句重复以“企业”起头。禁止使用“综上所述”“从……来看”“值得注意的是”“一方面……另一方面……”等模板化连接词；没有直接证据时禁止使用“领先”“优质”“实力雄厚”“有望”“预计”等评价或预测。
8. 四个编号标签由模板固定输出、加粗且不缩进；所有正文自然段首行缩进两个汉字。企业简介允许一至两个自然段，其余三项各一个自然段；历史字段缺失时不得留下空白占位段。
9. `EVIDENCE.core_company_profile`、`core_history_evolution`、`core_development_direction`、`core_risk_prompt`、`core_visit_objective` 分别逐句映射实际证据，`source_attributions.core` 汇总实际使用的内部维度与外部来源。任一字段验收失败时只重写该字段，不重新搜索、不补造事实；历史沿革验收失败则置为 `null`。
...
先完成内部原值、外部候选、来源正文、风险归一化和事实表，再按“内部主体描述 → 统一 OPP 台账及机会类型 → 统一 RISK 台账及风险类型 → 四段式核心观点 → 执行摘要 → 逐项营销方案 → 风险深化 → 按编号生成拜访脚本”的顺序生成。发现新证据时允许重建台账并同步全部编号引用。
