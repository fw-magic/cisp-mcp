...
7. 生成 `D.core_development_direction` 前，先对已证实的发展方向和实际行动逐项执行“合作相关性”判定。只有能回指至少一项 `opportunity_register` 中的 OPP，并能说明具体贷款用途、融资路径、结算资金流、现金管理、票据、收单、代发、供应链、跨境、投行、托管或其他综合金融服务场景的方向，才可进入本字段。纯精神口号、品牌理念、产品概念、组织宣导、公益叙事或其他无法映射具体合作场景的方向从核心观点省略。对通过筛选的内容，在同一自然段内同时回答“聚焦什么”和“已经采取什么行动”，保留时间、主体和执行状态边界。无任何合作相关方向，或无法同时核实方向与行动时，不补造经营规划，改为说明现有资料尚未形成可直接映射金融合作的重点发展动作，并列明拜访需核实的具体合作方向。
8. 生成 `D.core_risk_prompt` 前，对 `risk_register` 逐项执行“合作相关性”判定。符合以下任一条件才可进入核心观点：其一，`affected_opportunity_ids` 与 `D.core_opportunity_ids` 或本次拜访拟推进的 OPP 存在交集；其二，虽暂未绑定具体 OPP，但会影响主体资格、基础准入、合规、贷款用途、第一还款来源、现金流闭环、增信可用性、交易合规或合作推进节奏，属于企业级基础合作可行性风险。与合作无直接或间接影响的舆情细节、泛化行业评论、遥远关联主体或人物事项从核心观点省略，但不从 `risk_register`、风险章节或证据映射删除。本字段采用“合作相关风险事实＋由此需要关注的事项”写成一个自然段，不复制风险台账字段，不逐项堆列证据等级或核验清单。没有合作相关风险命中时，说明资料覆盖边界，并提出基础准入、合规、现金流或负债核验要求；不得据此作低风险判断。
...
10. 五个展示字段不得出现 OPP/RISK 编号、Markdown 标题、定位说明、来源名称或来源归因话术。禁止出现“企业官网披露”“据……”“……显示”“公开资料显示”“媒体报道”“平台信息”等表达。`D.core_opportunity_ids` 必须精确登记实际支持重点发展方向或拜访目标的 OPP，`D.core_risk_ids` 必须精确登记实际进入风险提示的 RISK；两者仅作为生成与证据验收的内部回指，完整编号索引继续由执行摘要承担。
...
14. `EVIDENCE.core_profile_candidate_facts` 逐项登记完整候选池和段落路由，`EVIDENCE.core_company_profile` 必须逐句回指第一段实际采用的 `CPF-xx` 及底层来源，`EVIDENCE.core_history_evolution` 必须逐句回指第二段实际采用的 `CPF-xx` 及底层来源；`EVIDENCE.core_development_direction` 逐句回指事实证据、对应 OPP 与合作场景，`EVIDENCE.core_risk_prompt` 逐句回指事实证据、对应 RISK、受影响 OPP 或企业级合作可行性影响，`EVIDENCE.core_visit_objective` 逐句映射实际证据，`source_attributions.core` 汇总实际使用的内部维度与 `Wn/Cn`。任一发展方向无法回指 OPP 或具体合作场景，任一核心风险无法回指受影响 OPP 或企业级合作可行性影响，第一段出现任何 `history_evolution` 内容，或第二段已生成但遗漏可得的成立时间，均验收失败。只重写对应核心字段，不重新搜索、不补造事实；历史沿革验收失败则置为 `null`。
