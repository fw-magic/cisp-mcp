...
1. `D.core_company_profile` 独立基于已通过主体、正文、时点和证据状态验收的内部事实与网络证据生成，不以 `D.core_internal_baseline` 为必选输入、起笔底座或生成前置。从企业定位、代表性荣誉、品牌、产品、许可、资质、业务基础和经营规模中选择最有区分度的事实形成一个自然段。企业名称、荣誉、品牌和量化数据必须有直接证据；宣传性称号不得改写为独立事实。
2. `D.core_history_evolution` 按时间先后组织成立、并购、重整、上市退市、重大股权变化、重大项目转折和当前状态，通常写成一个自然段。只有存在至少两个可以可靠排序的关键节点，且能说明当前状态时才生成；否则保持 `null`，骨架整段省略，不以资料不足句占位。
3. `D.core_development_direction` 必须在同一自然段内同时回答“当前聚焦什么”和“已经采取什么行动”。优先使用产品、渠道、品牌、组织、产能、采购、项目、数字化、门店或合作网络等企业专属动作，并保留时间、主体和执行状态边界。无法同时核实方向与行动时，不补造经营规划，改为说明现有资料尚不足以确认重点方向及落地动作，并列明拜访需核实的具体经营计划。
4. `D.core_risk_prompt` 采用“主要风险事实总结＋由此需要关注的事项”写成一个自然段，归纳相关 `risk_register` 事实、当前状态和可能影响，不复制风险台账字段，不逐项堆列证据等级或核验清单。没有明确风险命中时，说明内部与公开资料的覆盖边界，并提出基础准入、合规、现金流或负债核验要求；不得据此作低风险判断。
5. `D.core_visit_objective` 依据高相关 OPP 直接落到产能恢复或扩张、采购备货、渠道建设、项目投入、债务化解、经营结算、现金管理、票据、代发等贷款及综合金融服务场景，写明本次拜访希望确认的合作切入口。没有明确 OPP 时，改为融资用途、金额期限、还款来源、现有融资、增信资源和综合金融合作条件诊断；禁止写成“深入了解客户需求”等泛化目标。
6. 五个展示字段不得出现 OPP/RISK 编号、Markdown 标题、定位说明、来源名称或来源归因话术。禁止出现“企业官网披露”“据……”“……显示”“公开资料显示”“媒体报道”“平台信息”等表达。`D.core_opportunity_ids` 和 `D.core_risk_ids` 仅作为生成与证据验收的内部回指，完整编号索引继续由执行摘要承担。
7. 企业简介整体采用“简洁、专业、数据化、结论导向的银行对公企业研判简报文风”。定位段优先按“一句定位结论＋2 至 4 项可核验的规模、品牌、产品、资质或经营事实”组织；有可靠数据时优先保留年份、金额、数量、产能、门店、员工、项目规模等原始时点和单位。每句聚焦一个主要判断，不铺陈宣传口号、不写文学化沿革、不堆砌同义形容词。缺少可靠量化事实时使用简短定性事实，不为追求数据化而估算或补造数字。
8. 核心观点语言以企业名称、明确时间、具体动作和结果组织，避免连续句重复以“企业”起头。禁止使用“综上所述”“从……来看”“值得注意的是”“一方面……另一方面……”等模板化连接词；没有直接证据时禁止使用“领先”“优质”“实力雄厚”“有望”“预计”等评价或预测。
9. 四个编号标签由模板固定输出、加粗且不缩进；所有正文自然段首行缩进两个汉字。企业简介允许一至两个自然段，其余三项各一个自然段；历史字段缺失时不得留下空白占位段。
10. `EVIDENCE.core_company_profile`、`core_history_evolution`、`core_development_direction`、`core_risk_prompt`、`core_visit_objective` 分别逐句映射实际证据，`source_attributions.core` 汇总实际使用的内部维度与外部来源，两者均不进入核心观点报告正文。任一字段验收失败时只重写该字段，不重新搜索、不补造事实；历史沿革验收失败则置为 `null`。
