...
- `OPP-01`、`RISK-01`、`LEAD-01` 等编号，日期/“近期”等时点，以及“高｜已核验”“中｜线索”等短证据标签必须作为不可拆分的紧凑字段，不得逐字换行。内容确实超出单行时，只能在 `｜`、`/`、空格等语义分隔处主动换行，不得在任意汉字之间断行。紧凑列的表头与数据单元格均应应用等效的 `nowrap/keep-all` 策略，长叙述列不得全局禁止换行。
...
3. 核对开放来源覆盖：`WEB.queries` 保存全部查询，`WEB.candidates` 保存全部候选和打开状态，`WEB.sources` 保存可打开正文。没有交叉验证、主体间接、同名歧义、历史、无日期和正文未打开均不删除，只降低证据状态并进入 LEAD/WATCH/QUESTION。
...
6. 任一台账或来源验收不通过时，优先补充字段、降级证据状态或转入 QUESTION，不删除条目。发现新证据可以重新搜索、重建台账并同步编号；只对完全重复、无关、敏感或恶意内容执行删除。
...
9. 逐页渲染或预览最终 PDF，检查中文字体、Letter 页面、页边距、字号、行高、章节间距、表头样式、字重、列宽、自然分页、裁切、重叠和异常空白。逐表测量列边界，确认总宽度为 165.9 mm（允许 PDF 坐标换算误差 ±0.5 mm），每列不小于上述固定宽度且绝对不小于 18 mm。紧凑列中的编号、时点和短证据标签不得逐字换行；若出现 `OPP-01`、“近期”或“中｜线索”等内容竖排，即视为验收失败。逐表确认执行摘要主要机会与主要风险、企业基本信息、关键决策人、股权、资产、核心经营数据、行业表、融资需求线索、贷款产品候选、推进路径、风险预警、推荐话题、关键问题、资料清单和数据来源均按骨架显示，所有表头及跨页重复表头使用 `#D9EAF7` 实色底纹和中文粗体，普通数据行不使用该底纹；所有 OPP/RISK 编号使用中文粗体且清晰可辨。继续按固定规则验收报告使用说明、常规/粗体 PostScript 字形、字号和行高。除最后一页外，后续仍有连续正文但当前页空白超过可用正文区约三分之一时，视为分页失败并重新排版。
