...
- 正文段落：核心观点、执行摘要核心特征、主要机会与主要风险精简句、拜访建议、信息解读、贷款视角解读、普通说明、人员列表、产业与行业说明、营销边界和风险说明统一使用 15 pt 行高。核心观点、信息解读和贷款视角解读正文首行缩进 2 个汉字；执行摘要各项不缩进。主要机会与主要风险使用普通项目符号逐条展示，每条只含一句正文及句末编号，项目之间不插入空白段落。普通正文段落段后 4 pt；PDF 使用段后样式控制间距，不插入空白段落，Markdown 回退在自然段之间保留一个空行。
...
- 行内标签不得把带 `**` 的 Markdown 原样交给 ReportLab。只对骨架中的静态粗体标签以及摘要和表格中的 OPP/RISK 编号转换为实际中文粗体，动态正文先执行 XML 转义后再拼接；`信息解读：`、`贷款视角解读：`、`有形资产：`、`无形资产：`后的正文必须回到常规字体。
...
5. 检查内容质量：核心观点优先引用高相关 OPP/RISK，同时指向完整 OPP/RISK；执行摘要“主要机会”和“主要风险”均不得使用表格，必须按主台账顺序每个编号展示一句精简话，且不重复类型、证据、详细边界和核验清单；营销方案与风险章节保留详细表格，分别覆盖贷款和综合金融产品、交易条款、关联治理和行业市场。弱线索不得因不完整而丢失。
