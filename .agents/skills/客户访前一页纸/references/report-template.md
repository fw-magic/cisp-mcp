...
> 5. “核心观点”固定使用骨架内的四个加粗编号业务标签，名称和顺序不得改变；标签不是 Markdown 子标题且不缩进，正文自然段首行缩进两个汉字。“企业简介”包含企业定位段，有可靠关键沿革时再显示第二自然段；其余三项各显示一个自然段。核心观点不显示 OPP/RISK 编号。“执行摘要”固定展示核心特征、主要机会、主要风险和拜访建议，继续承担完整编号索引；主要机会和主要风险各自按主台账顺序使用 `1、2、3……` 连续列示，每条只展示一句精简话及句末 OPP/RISK 编号，不使用表格、不重复后文详细字段；机会类型、风险类型、证据、边界和核验信息在后续台账展示。
