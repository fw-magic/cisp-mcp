...
> 10. `D.company_overview_facts` 或 `D.company_overview_fallback` 验收失败时按确定性规则重新构建；`D.core_internal_baseline` 验收失败时只允许基于同一事实白名单重写一次，仍失败则使用回退文本。生成企业简介前必须验收 `D.core_profile_candidate_facts`：第一段合格候选存在但 `D.core_company_profile` 未回指任何 `CPF-xx`，存在定位/荣誉/品牌产品候选但第一段未采用该类候选，第一段出现成立时间或任何应路由到 `history_evolution` 的重整、股权、控制权、持股比例、经营管理权或管理层变化内容，或第二段已生成但未以可得的成立时间作为沿革起点，均判定失败并重写企业简介两段，禁止回退为工商公式段。其余核心观点展示字段分别验收，失败时只基于已通过证据验收的分项重写对应字段，禁止重新搜索或补造事实，`D.core_history_evolution` 仍失败时置为 `null` 并整段省略。条件裁剪后删除条件标签所在行并合并连续空白行，不得因沿革省略留下空白占位段。其他必填总结字段生成失败时才重写对应大模型派生字段。不得交付缺少四个核心观点标签及其必填正文、执行摘要或已显示小节信息解读的报告。
