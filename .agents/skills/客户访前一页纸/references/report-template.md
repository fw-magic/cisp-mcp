...
> 5. “核心观点”固定使用骨架内的四个加粗编号业务标签，名称和顺序不得改变；标签不是 Markdown 子标题且不缩进，正文自然段首行缩进两个汉字。“企业简介”包含企业定位段，有可靠关键沿革时再显示第二自然段；定位段必须从 `D.core_profile_candidate_facts` 选材，固定按“定位结论＋代表性荣誉/品牌/产品＋业务基础、经营规模或当前阶段”组织。存在合格候选时必须至少采用一项；存在定位、荣誉或品牌产品候选时必须采用其中至少一项。荣誉称号无需直接、高可靠或交叉验证证据，搜索摘要确有表述且主体直接对应即可使用；弱候选保留内部证据状态，正文不显示来源。成立时间、登记状态、注册资本、经营范围、注册地址、登记行业、法定代表人等工商字段为可选而非必显，不得固定作为简介底座。整个简介使用简洁、专业、数据化、结论导向的银行对公企业研判简报文风。其余三项各显示一个自然段。核心观点不显示 OPP/RISK 编号、证据来源名称或来源归因话术；来源仅保留在内部证据映射和文末数据来源汇总。“执行摘要”固定展示核心特征、主要机会、主要风险和拜访建议，继续承担完整编号索引；主要机会和主要风险各自按主台账顺序使用 `1、2、3……` 连续列示，每条只展示一句精简话及句末 OPP/RISK 编号，不使用表格、不重复后文详细字段；机会类型、风险类型、证据、边界和核验信息在后续台账展示。
...
> 10. `D.company_overview_facts` 或 `D.company_overview_fallback` 验收失败时按确定性规则重新构建；`D.core_internal_baseline` 验收失败时只允许基于同一事实白名单重写一次，仍失败则使用回退文本。生成企业简介前必须验收 `D.core_profile_candidate_facts`：合格候选存在但 `D.core_company_profile` 未回指任何 `CPF-xx`，或存在定位/荣誉/品牌产品候选但简介未采用该类候选，均判定失败并只重写企业简介，禁止回退为工商公式段。其余核心观点展示字段分别验收，失败时只基于已通过证据验收的分项重写对应字段，禁止重新搜索或补造事实，`D.core_history_evolution` 仍失败时置为 `null` 并整段省略。条件裁剪后删除条件标签所在行并合并连续空白行，不得因沿革省略留下空白占位段。其他必填总结字段生成失败时才重写对应大模型派生字段。不得交付缺少四个核心观点标签及其必填正文、执行摘要或已显示小节信息解读的报告。
