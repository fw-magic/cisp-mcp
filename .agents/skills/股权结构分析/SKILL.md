...
description: 对中国企业执行投资尽职调查和投前控制权核查，完成股东逐层穿透、实际控制人和最终受益人核验、一致行动线索与隐性关联关系识别、关联交易候选方排查，并生成带证据和置信度标识的可视化控制权结构图。默认交付 Letter 尺寸 PDF；运行环境明确标记为轻型模型、缺少可靠 PDF 生成与验收能力或 PDF 流程失败时，改为聊天内联完整 Markdown 报告。适用于用户要求分析目标公司股权架构、控制权、受益所有权、集团归属、关联方、共同投资或任职网络、股权变更与出质冻结风险时；通过连接标识为 cisp-mcp 的 MCP 工具获取事实。
---
...
使用方式：`/equity-structure-analysis 企业名称或统一社会信用代码 [--format pdf|md]`

- 默认格式：`pdf`
- Markdown 交付：聊天内联完整报告，不创建仅含路径或摘要的 `.md` 附件代替正文
- 报告定位：投资尽调与投前控制权核查，不构成法律意见或最终投资结论
...
先从同一份归一化图数据、路径、发现项和内部证据生成完整 Markdown 报告，再据此生成 PDF；不得让 Markdown 与 PDF 分别归纳原始响应。开始排版前按“交付模式”选择输出路径。

## 交付模式

1. 用户显式指定 `--format md` 时，直接在聊天中输出完整 Markdown 报告。
2. 默认使用 `--format pdf`。执行环境明确标记当前模型为轻型模型时，不尝试 PDF，直接交付聊天内联完整 Markdown 报告。
3. 没有明确模型标记时，检查当前模型和工具链是否同时具备：创建本地文件、注册中文常规与粗体字形、生成 PDF、把同一归一化图数据渲染为可核验矢量结构图、逐页渲染 PDF、检查页面图像和字体元数据。任一能力缺失即视为无法可靠生成 PDF，直接交付聊天内联完整 Markdown 报告。
4. 只有 PDF 能力检查全部通过时，才完整读取并严格执行 [PDF 交付规范](references/pdf-delivery.md)。用户显式要求 PDF 也不得绕过能力检查和验收。
5. PDF 生成、中文字体预检、结构图渲染、逐页预览或字体元数据验收任一步骤失败时，停止 PDF 流程，不切换多个渲染器反复重试；删除不完整 PDF 和本次临时文件，改为聊天内联完整 Markdown 报告。
6. Markdown 降级必须保留八章结构、四项报告头部、证据标签、Mermaid 图、全部必要表格、局限与待补证事项；不得只返回摘要、文件路径或部分章节。
7. PDF 验收通过后，聊天回复仅给出规范企业全称、查询时点、生成格式、最重要的 1 至 3 条资料范围说明和最终 PDF 的绝对路径链接。Markdown 模式则在聊天中直接给出完整正文。
