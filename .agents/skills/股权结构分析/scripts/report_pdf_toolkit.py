#!/usr/bin/env python3
"""股权结构分析 Skill 内置的 PDF 样式、字体、渲染与验收工具。"""
...
def make_action_list(
    actions: Sequence[object], styles: dict[str, ParagraphStyle]
) -> list[Paragraph]:
    """Render recommendation actions as one numbered item per visual line."""
    if len(actions) != 3:
        raise PdfContractError("建议行动必须恰好包含 3 条")
    rendered = [paragraph("建议行动：", styles["body"], bold=True)]
    rendered.extend(
        paragraph(f"{index}. {action}", styles["action"])
        for index, action in enumerate(actions, start=1)
    )
    return rendered


def _compact_cell_fits(value: object, width_mm: float) -> bool:
    """Keep compact styling only when the complete value fits on one line."""
    text = "" if value is None else str(value)
    if "\n" in text:
        return False
    available_width = width_mm * mm - 6  # Match the table's left/right padding.
    return pdfmetrics.stringWidth(text, REGULAR_ALIAS, TABLE_SIZE) <= available_width
    ...
    *,
    ...
    for row_index, row in enumerate(rows):
        ...
        for col_index, value in enumerate(row):
            ...
            elif col_index in compact and _compact_cell_fits(
                value, col_widths_mm[col_index]
            ):
